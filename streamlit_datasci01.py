...
def page2():
    ...
    with tab5:
        ...
        model = create_model(X.shape[1], l)
        ...
        # label_pred = st.empty()
        # label_pred.text("คุณยังไม่ได้กดปุ่ม Predict")
        lbl = st.text("Feature name to predicted:")
        label_fea = st.text(str(fea))
        xr = df[fea].values[0]
        txt = ""
        for x in xr[:-1]:
            txt += str(x)+","
        ...
        if len(lfea) == len(fea) :
            ...
            if pred:
                yr_pred = model.predict(xrstd)
                st.text("Predict Result : " + str(yr_pred[0]))
...
def page21():
    ...
    with tab1:
        ...
        #     txt1 += str(x)+","
        # txt1 += str(xr[-1])
        # real_log = st.text_input("Enter data feature to predicted",txt1) #"6.2,3.4,5.4,2.3"
        # # lfea = float(real_data)
        # lfea = list(map(float,real_log.split(",")))
    ...
    with tab2:
        ...
        #     txt2 += str(x)+","
        # txt2 += str(xr[-1])
        # real_svm = st.text_input("Enter data feature to predicted",txt2) #"6.2,3.4,5.4,2.3"
        # # lfea = float(real_data)
        # lfea = list(map(float,real_svm.split(",")))
    ...
    with tab3:
        ...
        #     txt3 += str(x)+","
        # txt3 += str(xr[-1])
        # real_knn = st.text_input("Enter data feature to predicted",txt3) #"6.2,3.4,5.4,2.3"
        # # lfea = float(real_data)
        # lfea = list(map(float,real_knn.split(",")))
    ...
    with tab4:
        ...
        #     txt4 += str(x)+","
        # txt4 += str(xr[-1])
        # real_dt = st.text_input("Enter data feature to predicted",txt4) #"6.2,3.4,5.4,2.3"
        # # lfea = float(real_data)
        # lfea = list(map(float,real_dt.split(",")))
    ...
    with tab5:
        ...
        #     txt5 += str(x)+","
        # txt5 += str(xr[-1])
        # real_rf = st.text_input("Enter data feature to predicted",txt5) #"6.2,3.4,5.4,2.3"
        # # lfea = float(real_data)
        # lfea = list(map(float,real_rf.split(",")))
    ...
    with tab6:
        ...
        classes = list(set(yy))
        # enc = OneHotEncoder()
        # yf = enc.fit_transform(yy[:, None]).toarray()
        n_features = len(fea)
        n_classes = len(classes)
        print(n_features,n_classes)
        # (Xk, Xkt, yk, ykt) = train_test_split(XX, yf,
        # test_size=0.25, random_state=101)
        ...
        model = create_model(n_features, n_classes, l)
        train = st.button("Train Model")
        label_res = st.empty()
        label_res.text("คุณยังไม่ได้กดปุ่ม Train")
        if train:
        ...
        if len(lfea) == len(fea) :
            ...
            pred = st.button("Predict Model")
            if pred:
                yr_pred = np.argmax(model.predict(xrstd),axis=1)
                st.text("Predict Result : " + str(yr_pred))
